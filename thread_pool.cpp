//
// Created by Shawn Zhao on 2023/7/17.
//
...
ThreadPool::ThreadPool() : sp_state_(std::make_shared<ThreadPool::State>()),
                         state_(sp_state_.get()), shutdown_on_destroy_(true) {
    pid_ = getpid();
}
...
int ThreadPool::GetCapacity() {
    ...
    // static_cast 是 C++ 中的一种类型转换操作符，用于执行静态类型转换。它可以将一个表达式或变量强制转换为指定的类型
}
